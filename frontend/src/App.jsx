...
function enrichProjectsWithMocks(items) {
  return items.map((project) => {
    const fallback = initialProjects.find((entry) => entry.title === project.title);
    return {
      ...fallback,
      ...project,
      requiredSkills: project.requiredSkills || fallback?.requiredSkills || [],
      interests: fallback?.interests || [],
      ownerGithub: fallback?.ownerGithub || "",
      ownerLinkedIn: fallback?.ownerLinkedIn || "",
    };
  });
}
...
function enrichStudentsWithMocks(items) {
  return items.map((student) => {
    ...
    return {
      ...
      password: "123456",
      name: student.name,
      department: student.department || fallback?.department || "",
      year: formatYearLabel(student.year || fallback?.year || ""),
...
function enrichAdvisorsWithMocks(items) {
  return items.map((advisor) => {
    ...
    const expertiseAreas = splitTextList(advisor.expertise || profileFallback?.expertiseAreas?.join(", "));
    const researchInterests =
      profileFallback?.researchInterests ||
      listFallback?.researchInterests ||
      splitTextList(advisor.research_interests);
    const supervisedProjects = profileFallback?.supervisedProjects || listFallback?.supervisedProjects || [];

    return {
      id: advisor.id,
      email: advisor.email || profileFallback?.email || "",
      role: "advisor",
      password: "123456",
      name: advisor.name,
      department: advisor.department || profileFallback?.department || "",
      title: advisor.title || profileFallback?.title || "Professor",
      expertise: expertiseAreas[0] || listFallback?.expertise || advisor.expertise || "General Advising",
      expertiseAreas,
      researchInterests,
      supervisedProjects,
      available: advisor.available ?? profileFallback?.available ?? true,
    };
  });
}
...
function enrichUserFromBackend(user) {
  ...
  if (user.role === "student") {
    ...
    return {
      ...
      year: formatYearLabel(user.year || fallback.year || ""),
      shortBio: user.bio || fallback.shortBio || "",
      github: user.github_link || fallback.github || "",
      linkedIn: user.linkedin_link || fallback.linkedIn || "",
      skills: fallback.skills || [],
      interests: fallback.interests || [],
    };
  }
  ...
  if (user.role === "advisor") {
    ...
    return {
      ...
      email: user.email,
      department: user.department || fallback.department || "",
      title: user.title || fallback.title || "Professor",
      expertiseAreas: splitTextList(user.expertise || fallback.expertiseAreas?.join(", ")),
      researchInterests: fallback.researchInterests || [],
      supervisedProjects: fallback.supervisedProjects || [],
...
function enrichAdvisorRequestsWithMocks(items) {
  return items.map((request) => {
    ...
    return {
      ...fallback,
      ...request,
      projectField: fallback?.projectField || request.type,
      description: fallback?.description || request.message || "No additional description provided.",
      objective: fallback?.objective || "No project objective shared yet.",
      deliverables: fallback?.deliverables || [],
      teamMembers: fallback?.teamMembers || 0,
      expectedDuration: fallback?.expectedDuration || "-",
      advisorPreference: fallback?.advisorPreference || "-",
  ...
  return applicationsByProject.flatMap(({ project, applications }) =>
    applications.map((application) => {
      ...
      return {
        ...
        note: "Application received through the backend.",
...
function App() {
  ...
  async function loadAppData(user) {
    ...
    if (user.role === "student") {
      const advisorItems = enrichAdvisorsWithMocks(await getAdvisors());
      ...
      const nextTeamRequests = buildTeamRequestsFromApplications(applicationBundles, studentProfiles);
    ...
    if (user.role === "advisor") {
      const nextRequests = enrichAdvisorRequestsWithMocks(await getRequests());
    ...
    if (user.role === "admin") {
      const [studentsFromApi, advisorsFromApi, advisorAccountsFromApi] = await Promise.all([
        getStudents(),
        getAdvisors(),
        getAdvisorAccounts(),
      ]);
      ...
      const nextAdvisors = enrichAdvisorsWithMocks(advisorsFromApi);
  ...
  async function handleStudentProfileSave(profileUpdates) {
    ...
    try {
      await updateMe({
        ...
        year_level: profileUpdates.year,
  ...
  async function handleAdvisorRequest(advisor) {
    const ownedProject = studentProjects[0] || selectedStudentProject;
    if (!ownedProject) {
  ...
  async function handleApplyToProject(project) {
  ...
  function handleAdminProjectSave(id, projectUpdates) {
    const previousProject = projects.find((project) => project.id === id);
    if (!previousProject) return;

    const nextProject = {
      ...previousProject,
      title: projectUpdates.title || previousProject.title,
      owner: projectUpdates.owner || previousProject.owner,
      advisor: projectUpdates.advisor || previousProject.advisor,
      type: projectUpdates.type || previousProject.type,
      status: projectUpdates.status || previousProject.status,
      description: projectUpdates.description || previousProject.description,
      teamMembers: Number(projectUpdates.teamMembers || previousProject.teamMembers || 0),
      requiredSkills: normalizeCommaSeparatedList(projectUpdates.requiredSkills),
      interests: normalizeCommaSeparatedList(projectUpdates.interests),
      ownerGithub: projectUpdates.ownerGithub || previousProject.ownerGithub || "",
      ownerLinkedIn: projectUpdates.ownerLinkedIn || previousProject.ownerLinkedIn || "",
    };

    setProjects(projects.map((project) => (project.id === id ? nextProject : project)));
    setMessage(`Project "${nextProject.title}" updated successfully.`);
  }
        ...
        {activeStudentProject ? (
          <div className="admin-panel-overlay">
            <div className="admin-panel-sheet">
              ...
              <ProjectDetailsCard project={activeStudentProject} embedded />
            </div>
          </div>
        ) : null}
      </>
          ...
          {advisors.map((advisor) => (
            <div className="advisor-row advisor-request-row" key={advisor.id}>
              ...
              <div className="advisor-request-actions">
                ...
                <button className="primary-btn" type="button" onClick={() => onAdvisorRequest(advisor)}>
                  Send Request
                </button>
              </div>
            </div>
...
function ProjectDetailsCard({ project, showOwnerProfile = false, studentDirectory = [], embedded = false }) {
